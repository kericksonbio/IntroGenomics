...
-   Interpret figures generated from genomic data
-   Generate and customize histograms, scatterplots and boxplots in R 
-   Become familiar with a suite of plotting functions and parameters in R

## Warmup: Interpreting Figures
For this warmup, you'll be analyzing two figures below, from which we have omitted 
some important information! These are modified from real scientific papers in the 
field of genomics.

Today, we will ask you to interpret what you can about these figures and use your 
imagination to predict what kind of information they might be showing. 
Here's how our warmup will be structured:

-   2 minutes to look at both figures individually 
-   3 minutes to discuss the figures with a partner
-   3 minutes to share with the class
-   3 minutes to discuss the figures with all of the context

Here are some questions to guide your thoughts and conversations:

-   What patterns do you see?
-   What elements are missing that you'd need to interpret this figure?
-   What are the potential meanings of the axes and shapes?
-   What do you think the main message of the figure is?
  
### Figure 1

![](./figs/w6_warmup_1.jpg)


### Figure 2

![](./figs/w6_warmup_2.jpg)

<details>
<summary><b>After the warmup,</b> click here to see the complete versions of each figure</summary>
<p> Figures 1 and 2B from <a href="https://academic.oup.com/mbe/article/35/11/2639/5068378"> Brennan <i> et al., </i>2018</a> 
"Integrative Population and Physiological Genomics Reveals Mechanisms of Adaptation in Killifish"

<img src="./figs/w6_warmup_full_fig1.jpg" alt="Figures 1 and 2B from Brennan et al., 2018">

Figure from <a href="https://www.science.org/doi/full/10.1126/science.aan4380"> Bay <i> et al.,</i> 2018</a> 
"Genomic signals of selection predict climate-driven population declines in a migratory bird"

<img src="./figs/w6_warmup_full_fig2.jpg" alt="Figure 2A from Bay at al., 2018 ">
</p>
</details>


## Data Manipulation
Today, we're going to work with a sleep data set which is available in R. 
Start an RStudio OnDemand session, and make a new R script titled `Week6plotting` 
so that you can follow along with today's code! We will again work with the sleep
...
```{r, echo=T, eval=FALSE}
# install.packages("ggplot2") #only needs to be run once, uncomment if needed
library(ggplot2)
data("msleep")
```

> ## Class Exercise 1: Getting oriented with the sleep dataset
>
> 1. What is mean of the `sleep_total` column?
>
> 2. What are the max and min of the `sleep_total` column?
>
> 3. Which animal is represented in the 3rd row of the dataframe?
> 
> Challenge Questions:
>
> 4. Can you use one command to see how many each of carnivores, herbivores, 
> insectivores and omnivores are in this dataset?
>
> 5. Which carnivore gets the most sleep? How many hours?


Okay, now we're moving onto some new ways to make the most of our dataframe!
...
Sometimes, the values we care about aren't directly available in a data set. When
this happens, we can create a new column that contains the values we're
interested in:

```{r, echo=T, eval=FALSE}

  # what if what we cared about was our sleep_rem/sleep_total ratio?
  # add a sleep_rem/sleep_total ratio column to our msleep dataframe with $
msleep$rem_total<-msleep$sleep_rem/msleep$sleep_total
...
Sometimes we use logical expressions to subset our dataframes. Here's a list for 
your reference:
...
# for each row in our dataframe, does that species sleep greater than or equal to 18 hours?
msleep[,"sleep_total"]>=18

# we can use which() to identify which rows match the logical values (TRUE) and length to count how many species there are
...
>
> 1. How many mammals sleep 6 hours or less total? Of these species, how many are marine mammals (in the order Cetacea)?
>
> 2. Of the mammals that less less than 6 hours or less total, how many of them are carnivores?
>
> 3. Identify which mammal(s) sleep the average amount of hours you sleep at night!
...
```{r,  label='6-0', echo=T}
# now switch, the order of the variables and see how the plot differs  
plot(msleep$sleep_rem~msleep$sleep_total)
```
...
> `sleep_total` is on the x (independent variable) and `sleep_rem` is on
...
-   `col` changes color
-   `pch` changes point character (symbol)
-   `cex` changes size
...
cheatsheat
[here](<https://rstudio.github.io/cheatsheets/html/data-visualization.html>)
...
same and is even more noticeible in ggplot is the iterative process of
...
and I want to see how they relate to my continuous variables. Let's return 
to the iris dataset that we introduced in the Week 5 activity. 
...
## Group Work Activity: Plotting with Palmer Penguins!

![Artwork by](./figs/lter_penguins.png) For this week's activity we will
be working with the dataset [palmerpenguins](https://allisonhorst.github.io/palmerpenguins) First install the
...
1)  Using the `penguins` dataset, how does bill length vary across the
...
2)  Again using the `penguins` dataset visualize the relationship
...
This week, rather than submitting code, please submit screenshots or
downloaded image files of your graphs. You can submit the interpretation of the 
graphs wither as screenshots/images or as text.
...
#Q5
max_carni_sleep = max(na.omit(msleep$sleep_total[msleep$vore=="carni"], ))
msleep[msleep$sleep_total==max_carni_sleep & msleep$vore=="carni",]
msleep$name[msleep$sleep_total==max_carni_sleep & msleep$vore=="carni"]
